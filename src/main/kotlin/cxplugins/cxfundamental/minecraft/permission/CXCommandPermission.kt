...
class CXCommandPermission {
    ...
    fun permission(lambda: CXCommandPermission.() -> Unit) {
        ...
        permissionMap["$root.${commandPermission.name}"] =
            Permission(root + "." + commandPermission.name, commandPermission.description, commandPermission.default)
        for (entry in commandPermission.permissionMap.entries) {
            this.permissionMap[entry.key] = entry.value
        }
    }
}