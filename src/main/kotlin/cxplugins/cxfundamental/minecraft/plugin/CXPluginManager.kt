    ...
    init{
        ...
        for(plugin in plugins){
            if(plugin is CXPluginMain){
                ...
                item!!.appendLore("&4&l右键点我+下蹲删除此插件")
                var button=object: CXButton(item!!) {
                    ...
                    override fun onLeftClick(event: InventoryClickEvent, frame:CXFrame) {
                        ...
                        if (Bukkit.getPluginManager().isPluginEnabled(plugin)) {
                            ...
                            owner.sendMessageWithColor("&2&l[CXFundamental]操作成功 此插件已经被关闭")
                        } else {
                            owner.sendMessageWithColor("&4&l[CXFundamental]你无法关闭一个已经被关闭的插件")
                        }
                        return
                    }
                    ...
                    override fun onRightClick(event: InventoryClickEvent, frame:CXFrame) {
                        ...
                        if (!Bukkit.getPluginManager().isPluginEnabled(plugin)) {
                            ...
                            owner.sendMessageWithColor("&2&l[CXFundamental]操作成功 此插件已经开启")
                        } else {
                            owner.sendMessageWithColor("&4&l[CXFundamental]你无法开启一个正在运行的插件")
                        }
                        return
                    }
                    ...
                    override fun onLeftShiftClick(event: InventoryClickEvent, frame:CXFrame) {
                        ...
                        try {
                            ...
                            owner.sendMessageWithColor("&4&l[CXFundamental]此插件不包含重载功能")
                            return
                        }
                        owner.sendMessageWithColor("&2&l[CXFundamental]操作成功 此插件已经重载")

                    }

                    override fun onRightShiftClick(event: InventoryClickEvent, frame: CXFrame) {
                        super.onRightShiftClick(event, frame)
                        val optionPane=object:CXOptionPane(1,"&4你确定要删除此插件吗?",CXOptionPaneButtonType.CONFIRM,CXOptionPaneButtonType.CANCEL){
                            override fun onClickConfirm(event: InventoryClickEvent, frame: CXFrame) {
                                ...
                                if(File(".\\plugins\\${plugin.name}.jar").exists()){
                                    ...
                                    owner.sendMessageWithColor("&2&l[CXFundamental]操作成功 此插件已经被删除")
                                }
                                else{
                                    owner.sendMessageWithColor("&4&l[错误]无法找到此插件 请确定此插件的文件名是否为 <插件名>.jar")
                                }
            ...
            val button=object :CXButton(){
                ...
                override fun onLeftClick(event: InventoryClickEvent, frame: CXFrame) {
                    ...
                    if(pluginName in pluginNames){
                        val optionPane=object:CXOptionPane(1,"&4&l此插件已经存在 你确认要重新下载它吗?",CXOptionPaneButtonType.CONFIRM,CXOptionPaneButtonType.CANCEL){
                            override fun onClickConfirm(event: InventoryClickEvent, frame: CXFrame) {
                                super.onClickConfirm(event, frame)
