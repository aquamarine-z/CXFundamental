...
open class CXPanel(val panelWidth: Int, val panelHeight: Int) : CXContainer {
    ...
    override fun getBaseComponent(x: Int, y: Int): CXComponent? {
    ...
    override fun getBaseComponent(location: Int): CXComponent? {
        return null
    }
        ...
        for (i in elementArray.indices) {
            for (j in elementArray[i].indices) {
                if (elementArray[i][j] != null) {
                    val array = elementArray[i][j]?.repaint(0, 0)
                    if (array != null) {
                        for (kx in array.indices) {
                            for (ky in array[kx].indices) {
                                if (array[kx][ky] != null) {
                                    items[i + kx][j + ky] = array[kx][ky]
                                }
                            }
                        }
                    }
